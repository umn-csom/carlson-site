...
var numberFormat = Templating.numberFormat;
...
var addEvent = U.addEvent, attr = U.attr, createElement = U.createElement, css = U.css, defined = U.defined, diffObjects = U.diffObjects, discardElement = U.discardElement, erase = U.erase, error = U.error, extend = U.extend, find = U.find, fireEvent = U.fireEvent, getAlignFactor = U.getAlignFactor, getStyle = U.getStyle, isArray = U.isArray, isNumber = U.isNumber, isObject = U.isObject, isString = U.isString, merge = U.merge, objectEach = U.objectEach, pick = U.pick, pInt = U.pInt, relativeLength = U.relativeLength, removeEvent = U.removeEvent, splat = U.splat, syncTimeout = U.syncTimeout, uniqueKey = U.uniqueKey;
/* *
 *
 *  Class
 *
...
var Chart = /** @class */ (function () {
    ...
    Chart.prototype.setZoomOptions = function () {
        var chart = this, options = chart.options.chart, zooming = options.zooming;
        chart.zooming = __assign(__assign({}, zooming), { type: pick(options.zoomType, zooming.type), key: pick(options.zoomKey, zooming.key), pinchType: pick(options.pinchType, zooming.pinchType), singleTouch: pick(options.zoomBySingleTouch, zooming.singleTouch, false), resetButton: merge(zooming.resetButton, options.resetZoomButton) });
    };
    /**
    ...
    Chart.prototype.getClipBox = function (series, chartCoords) {
        var _a;
        var _b, _c, _d, _e;
        var inverted = this.inverted, _f = series || {}, xAxis = _f.xAxis, yAxis = _f.yAxis;
        // If no axes on the series, or series undefined, use global clipBox
        var _g = merge(this.clipBox), x = _g.x, y = _g.y, width = _g.width, height = _g.height;
        if (series) {
    ...
    Chart.prototype.isInsidePlot = function (plotX, plotY, options) {
        ...
        var _b = this, inverted = _b.inverted, plotBox = _b.plotBox, plotLeft = _b.plotLeft, plotTop = _b.plotTop, scrollablePlotBox = _b.scrollablePlotBox, _c = (options.visiblePlotOnly &&
            ((_a = this.scrollablePlotArea) === null || _a === void 0 ? void 0 : _a.scrollingContainer)) || {}, _d = _c.scrollLeft, scrollLeft = _d === void 0 ? 0 : _d, _e = _c.scrollTop, scrollTop = _e === void 0 ? 0 : _e, series = options.series, box = (options.visiblePlotOnly && scrollablePlotBox) || plotBox, x = options.inverted ? plotY : plotX, y = options.inverted ? plotX : plotY, e = {
            x: x,
            y: y,
            isInsidePlot: true,
            options: options
    ...
    Chart.prototype.createAxes = function () {
        ...
        for (var _i = 0, _a = ['xAxis', 'yAxis']; _i < _a.length; _i++) {
            var coll = _a[_i];
            var arr = options[coll] = splat(options[coll] || {});
    ...
    Chart.prototype.applyDescription = function (key, explicitOptions) {
        ...
        var options = this.options[key] = merge(this.options[key], explicitOptions);
        var elem = this[key];
    ...
    Chart.prototype.layOutTitles = function (redraw) {
        ...
        var titleOffset = [0, 0, 0], _e = this, options = _e.options, renderer = _e.renderer, spacingBox = _e.spacingBox;
        // Lay out the title, subtitle and caption respectively
        ['title', 'subtitle', 'caption'].forEach(function (key) {
            var _a;
            var desc = _this[key], descOptions = _this.options[key], alignTo = merge(spacingBox), textPxLength = (desc === null || desc === void 0 ? void 0 : desc.textPxLength) || 0;
            if (desc && descOptions) {
                // Provide a hook for the exporting button to shift the title
                fireEvent(_this, 'layOutTitle', { alignTo: alignTo, key: key, textPxLength: textPxLength });
                ...
                desc
                    ...
                    scaleX: scale,
                    scaleY: scale,
                    'transform-origin': "".concat(alignTo.x +
                        textPxLength *
                            scale *
                            getAlignFactor(alignAttr.align), " ").concat(lineHeight)
                });
    ...
    Chart.prototype.getContainerBox = function () {
        ...
        var nonContainers = [].map.call(this.renderTo.children, function (child) {
    ...
    Chart.prototype.getChartSize = function () {
        var _a;
        var chart = this, optionsChart = chart.options.chart, widthOption = optionsChart.width, heightOption = optionsChart.height, containerBox = chart.getContainerBox(), enableDefaultHeight = containerBox.height <= 1 ||
    ...
    Chart.prototype.getContainer = function () {
        var _a, _b;
        var chart = this, options = chart.options, optionsChart = options.chart, indexAttrName = 'data-highcharts-chart', containerId = uniqueKey(), renderTo = chart.renderTo;
        var containerStyle;
    ...
    Chart.prototype.getMargins = function (skipAxes) {
        var _a;
        var _b = this, spacing = _b.spacing, margin = _b.margin, titleOffset = _b.titleOffset;
        this.resetMargins();
    ...
    Chart.prototype.getAxisMargins = function () {
        ...
        marginNames.forEach(function (marginName, side) {
    ...
    Chart.prototype.reflow = function (e) {
        var _a, _b;
        var chart = this, oldBox = chart.containerBox, containerBox = chart.getContainerBox();
    ...
    Chart.prototype.setReflow = function () {
        ...
        if (typeof ResizeObserver === 'function') {
            ...
            var unbind = addEvent(win, 'resize', runReflow);
    ...
    Chart.prototype.setSize = function (width, height, animation) {
        ...
        chart.getChartSize();
        var chartWidth = chart.chartWidth, chartHeight = chart.chartHeight, _a = chart.scrollablePixelsX, scrollablePixelsX = _a === void 0 ? 0 : _a, _b = chart.scrollablePixelsY, scrollablePixelsY = _b === void 0 ? 0 : _b;
        ...
        if (chart.isDirtyBox ||
            ...
            if (!chart.styledMode) {
                (globalAnimation ? animate : css)(chart.container, {
                    width: "".concat(chartWidth + scrollablePixelsX, "px"),
                    height: "".concat(chartHeight + scrollablePixelsY, "px")
                }, globalAnimation);
            }
    ...
    Chart.prototype.setChartSize = function (skipAxes) {
        var _a, _b;
        var chart = this, chartHeight = chart.chartHeight, chartWidth = chart.chartWidth, inverted = chart.inverted, spacing = chart.spacing, renderer = chart.renderer, clipOffset = chart.clipOffset, clipRoundFunc = Math[inverted ? 'floor' : 'round'];
    ...
    Chart.prototype.resetMargins = function () {
        ...
        var chart = this, chartOptions = chart.options.chart, plotBorderWidth = chartOptions.plotBorderWidth || 0, halfWidth = Math.round(plotBorderWidth) / 2;
        ...
        ['margin', 'spacing'].forEach(function (target) {
            ...
            [
                ...
                var _a;
                chart[target][side] = ((_a = chartOptions["".concat(target).concat(sideName)]) !== null && _a !== void 0 ? _a : values[side]);
            });
        });
        ...
        marginNames.forEach(function (marginName, side) {
            var _a;
            chart[marginName] = (_a = chart.margin[side]) !== null && _a !== void 0 ? _a : chart.spacing[side];
        });
    ...
    Chart.prototype.linkSeries = function (isUpdating) {
        ...
        fireEvent(this, 'afterLinkSeries', { isUpdating: isUpdating });
    };
    /**
    ...
    Chart.prototype.render = function () {
        var _a;
        var chart = this, axes = chart.axes, colorAxis = chart.colorAxis, renderer = chart.renderer, axisLayoutRuns = chart.options.chart.axisLayoutRuns || 2, renderAxes = function (axes) {
            axes.forEach(function (axis) {
                if (axis.visible) {
                    axis.render();
                }
        ...
        for (var _i = 0, axes_1 = axes; _i < axes_1.length; _i++) {
            var axis = axes_1[_i];
            var options = axis.options, labels = options.labels;
            if (chart.hasCartesianSeries && // #20948
                ...
                var mockTick = new Tick(axis, 0, '', true), label = mockTick.createLabel('x', labels);
                mockTick.destroy();
                if (label &&
        ...
        while ((redoHorizontal || redoVertical || axisLayoutRuns > 1) &&
            ...
            var tempWidth = chart.plotWidth, tempHeight = chart.plotHeight;
    ...
    Chart.prototype.destroy = function () {
        var _a, _b;
        var chart = this, axes = chart.axes, series = chart.series, container = chart.container, parentNode = container === null || container === void 0 ? void 0 : container.parentNode;
        var i;
    ...
    Chart.prototype.firstRender = function () {
        ...
        var series = isArray(options.series) ? options.series : [];
    ...
    Chart.prototype.createAxis = function (coll, options) {
        var axis = new Axis(this, options.axis, coll);
    ...
    Chart.prototype.showResetZoom = function () {
        var chart = this, lang = defaultOptions.lang, btnOptions = chart.zooming.resetButton, theme = btnOptions.theme, alignTo = (btnOptions.relativeTo === 'chart' ||
    ...
    Chart.prototype.zoomOut = function () {
        var _this = this;
        fireEvent(this, 'selection', { resetSelection: true }, function () { return _this.transform({ reset: true, trigger: 'zoom' }); });
    };
    /**
    ...
    Chart.prototype.pan = function (event, panning) {
        var chart = this, panningOptions = (typeof panning === 'object' ?
            panning :
            {
                enabled: panning,
                type: 'x'
        ...
        fireEvent(this, 'pan', { originalEvent: event }, function () {
            chart.transform({
                axes: axes,
                event: event,
                to: {
    ...
    Chart.prototype.transform = function (params) {
        ...
        var _this = this;
        var _b, _c, _d;
        var _e = params.axes, axes = _e === void 0 ? this.axes : _e, event = params.event, _f = params.from, from = _f === void 0 ? {} : _f, reset = params.reset, selection = params.selection, _g = params.to, to = _g === void 0 ? {} : _g, trigger = params.trigger, _h = this, inverted = _h.inverted, time = _h.time;
        ...
        var hasZoomed = params.hasZoomed || false, displayButton, isAnyAxisPanning;
        for (var _i = 0, axes_3 = axes; _i < axes_3.length; _i++) {
            var axis = axes_3[_i];
            var horiz = axis.horiz, len = axis.len, _j = axis.minPointOffset, minPointOffset = _j === void 0 ? 0 : _j, options = axis.options, reversed = axis.reversed, wh = horiz ? 'width' : 'height', xy = horiz ? 'x' : 'y', toLength = pick(to[wh], axis.len), fromLength = pick(from[wh], axis.len), 
            ...
            var newMin = eventMin + offset, newMax = eventMax - offset, allExtremes = axis.allExtremes;
            if (selection) {
            ...
            var _m = extend(axis.getExtremes(), allExtremes || {}), dataMin = _m.dataMin, dataMax = _m.dataMax, min = _m.min, max = _m.max, optionsMin = time.parse(options.min), optionsMax = time.parse(options.max), 
