...
(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.behaviors.select2 = {
    ...
    attach: function (context, settings) {
      // Attach Select2 behavior to the elements.
      once('select2', this.getElements(context)).forEach(function (element) {
        this.createSelect2(element);
    
        // Add accessibility label for dynamically generated search input.
        const select2Container = $(element).next('.select2-container');
        const searchInput = select2Container.find('.select2-search__field');
    
        if (searchInput.length) {
          // Find the label text associated with this <select> element.
          const labelElement = $(element).closest('form').find(`label[for="${$(element).attr('id')}"]`);
          const labelText = labelElement.text().trim();
    
          // Add the label text as aria-label to the input field.
          if (labelText) {
            searchInput.attr('aria-label', labelText);
          }
        }
      }.bind(this));
    },
