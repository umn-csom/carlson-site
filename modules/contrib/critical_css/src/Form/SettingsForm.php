...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['critical_css_enabled'] = [
      ...
      '#description' => $this->t(
        "Enable Critical CSS. Drupal cache must be rebuilt when this value changes."
      ),
    ];
    ...
    $form['critical_css_dir_path'] = [
      '#type' => 'textfield',
      '#title' => $this->t(
        'Critical CSS base directory (relative to @theme_path)',
        [
          '@theme_path' => $this->themeExtensionList->getPath(
    ...
    $form['critical_css_excluded_ids'] = [
      ...
      '#required' => FALSE,
      '#description' => $this->t(
        'Enter ids of entities (one per line) which should not be processed. These entities will load standard CSS (synchronously).'
      ),
    ...
    $form['critical_css_advanced']['critical_css_enabled_for_logged_in_users'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Enabled for logged-in users'),
      '#default_value' => $config->get('enabled_for_logged_in_users'),
      '#description' => $this->t(
        "This option will enable Critical CSS for logged-in users. Since the contents of the critical CSS files are generated emulating an anonymous visit, it is not recommended to enable this option. Keep in mind that Critical CSS is always disabled for admin routes (i.e.- the ones under /admin) to avoid several conflicts with admin themes."
      ),
    ];

    $form['critical_css_advanced']['critical_css_preload_non_critical_css'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Preload non-critical CSS'),
      '#default_value' => $config->get('preload_non_critical_css'),
      '#description' => $this->t(
        "Browsers will load non-critical CSS files with the highest priority. Not recommended unless the site is experiencing FOUC (Flash Of Unstyled Content) issues."
      ),
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (strpos($criticalCssDirPath, '..') !== FALSE) {
      $form_state->setErrorByName(
        'critical_css_dir_path',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config
      ...
      ->set(
        'enabled_for_logged_in_users',
        $form_state->getValue('critical_css_enabled_for_logged_in_users')
      )
      ->set(
        'preload_non_critical_css',
        $form_state->getValue('critical_css_preload_non_critical_css')
      )
